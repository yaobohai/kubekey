...
func (d *DisableContainerd) Execute(runtime connector.Runtime) error {
	...
	if d.KubeConf.Cluster.Registry.DataRoot != "" {
		...
		files = append(files, "/data/containerd/data")
	}
...
func (i *EditKubeletCri) Execute(runtime connector.Runtime) error {
	...
	case common.Docker:
		if _, err := runtime.GetRunner().SudoCmd(fmt.Sprintf(
			"sed -i 's#--container-runtime=remote --container-runtime-endpoint=unix:///data/containerd/state/containerd.sock --pod#--pod#' /var/lib/kubelet/kubeadm-flags.env"),
			true); err != nil {
			return errors.Wrap(err, "Change KubeletTo Containerd failed")
		}
	case common.Containerd:
		if _, err := runtime.GetRunner().SudoCmd(fmt.Sprintf(
			"sed -i 's#--network-plugin=cni --pod#--network-plugin=cni --container-runtime=remote --container-runtime-endpoint=unix:///data/containerd/state/containerd.sock --pod#' /var/lib/kubelet/kubeadm-flags.env"),
			true); err != nil {
			return errors.Wrap(err, "Change KubeletTo Containerd failed")
		}
